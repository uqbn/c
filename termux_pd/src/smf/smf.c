...
int pd_conf(Pd_data *const pd_data, const char *const confname)
{
    if (confname == NULL)
        return -1;
    ...
    cJSON *confjson = cJSON_Parse(conf);
    free(conf);

    if (confjson == NULL)
        return -2;

    cJSON *itemdata = cJSON_GetObjectItem(confjson, PD_DATA);
    if (itemdata != NULL)
    {
        conf = itemdata->valuestring;
        pd_data->dataname = (char *)calloc(strlen(conf) + 1, sizeof(char));
        strcpy(pd_data->dataname, conf);
    }
